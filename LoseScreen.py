...
class LoseScreen:
    ...
    def lScreenLoop(called):

        bgImg = pygame.image.load('gameover-screen.png')
        ...
        button_x = (width - button_width) / 2 + 300
        button_y = (height - button_height) / 2 + 200
        color = (255,255,255)
        color_light = (170,170,170)
        color_dark = (100,100,100)

        quitBtn = Button(screen, color_light, color_dark, button_x-600, button_y, button_width, button_height, 'Quit', color, 'FORCED SQUARE.ttf', 35)
        startBtn = Button(screen, color_light, color_dark, button_x, button_y, button_width, button_height, 'Try Again', color, 'FORCED SQUARE.ttf', 35)
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    sys.exit()
                        ...
                        if startBtn.is_clicked(mouse):
                            running = False

                        elif quitBtn.is_clicked(mouse):
                            running = False
                            sys.exit()
            render()
